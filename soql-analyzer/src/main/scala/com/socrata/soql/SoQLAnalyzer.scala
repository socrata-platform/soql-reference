                                ...
                                parserParameters: AbstractParser.Parameters = AbstractParser.defaultParameters)
{
  ...
  type AnalysisContext = com.socrata.soql.AnalysisContext[Type, Value]
  type SchemaContext = Map[Qualifier, DatasetContext[Type]]
  ...
  private def baseAnalysis(implicit ctx: AnalysisContext) = {
    ...
    val selections = ctx.schemas.map { case (qual, dsCtx) =>
    ...
    SoQLAnalysis(isGrouped = false,
                 distinct = typed.Indistinct[ColumnName, Type](),
  ...
  /** Turn framents of a SoQL SELECT statement into a typed `Analysis` object.  If no `selection` is provided,
    ...
    *
    * @param distinctive   Reduce identical tuples into one.
  ...
  def analyzeSplitQuery(distinct: Option[String],
    ...
    def dispatch(distinct: Distinctiveness, selection: Option[Selection],
                 joins: List[Join],
    ...
    dispatch(
      distinct.map(p.distinct).getOrElse(Indistinct),
      ...
      offset.map(p.offset),
      search.map(p.search),
      hints.map(p.hints).toList.flatten
    )
  }
  ...
  def analyzeNoSelectionInOuterSelectionContext(lastQuery: Analysis,
    ...
    implicit val fakeCtx = AnalysisContext[Type, Value](schemas = schemasFromAnalysis(lastQuery), parameters = ParameterSpec.empty)
  ...
  private def joinCtx(joins: Seq[Join], parentFromContext: SchemaContext)(implicit ctx: AnalysisContext): SchemaContext = {
    joins.foldLeft(parentFromContext) { (acc, join) =>
      val jCtx = if (join.lateral) ctx.schemas ++ acc else ctx.schemas
      join.from match {
        case JoinQuery(selects, alias) =>
          validateAlias(alias)
          val analyses = analyzeBinary(selects)(ctx.copy(schemas = jCtx))
          acc + schemasFromAnalysis(alias, analyses.outputSchema.leaf)
  ...
  def analyzeNoSelection(distinct: Distinctiveness,
                         joins: List[Join],
                         where: Option[Expression],
                         groupBys: List[Expression],
                         having: Option[Expression],
                         ...
                         offset: Option[BigInt],
                         search: Option[String],
                         hints: List[Hint])(implicit ctx: AnalysisContext): Analysis = {
    ...
    val ctxWithJoins = ctx.withUpdatedSchemas(_ ++ ctxFromJoins)
    ...
    val checkedDistinct = distinct match {
      case Indistinct => typed.Indistinct[ColumnName, Type]()
      case FullyDistinct => typed.FullyDistinct[ColumnName, Type]()
      case DistinctOn(exprs) => typed.DistinctOn(exprs.map(typecheck))
    }
    ...
    if(log.isTraceEnabled) {
      ...
      log.trace("typechecking for DISTINCT took {}ms", ns2ms(t6 - t5))
    }
    ...
    val (names, typedSelectedExpressions) = if(isGrouped) {
      ...
      val names = AliasAnalysis(Selection(None, Seq.empty, untypedSelectedExpressions.map(SelectedExpression(_, None))), None)(ctx.schemas).expressions.keys.toSeq
      ...
      val beforeAliasAnalysis = System.nanoTime()
      val names = AliasAnalysis(Selection(None, Seq(StarSelection(None, Nil)), Nil), None)(ctx.schemas).expressions.keys.toSeq
      ...
      val typedSelectedExpressions = names.map { column =>
        typed.ColumnRef(None, column, ctx.schemas(TableName.PrimaryTable.qualifier).schema(column))(NoPosition)
      }
    ...
    val checkedHint = hints.map { h: Hint =>
      typechecker.typecheckHint(h, Map.empty, None)
    }
  ...
  def analyzeInOuterSelectionContext(initialCtx: AnalysisContext)(lastQuery: Analysis, query: Select): Analysis = {
    val prevCtx = schemasFromAnalysis(lastQuery)
    val nextCtx = prevCtx ++ initialCtx.schemas.view.filterKeys(qualifier => TableName.PrimaryTable.qualifier != qualifier)
    analyzeWithSelection(query)(initialCtx.copy(schemas = nextCtx))
  }

  def schemasFromAnalysis(a: Analysis): SchemaContext = {
  ...
  private def schemasFromAnalysis(qualifier: Qualifier, a: Analysis) = {
    val ctx = new DatasetContext[Type] {
      override val schema: OrderedMap[ColumnName, Type] = a.selection.withValuesMapped(_.typ)
    }
    (qualifier -> ctx)
  ...
  private def aliasContext(tableName: Option[TableName], ctx: SchemaContext): SchemaContext = {
    tableName match {
  ...
  private def aliasContext(from: TableName, ctx: SchemaContext): SchemaContext = {
  ...
  def analyzeWithSelection(query: Select)(implicit ctx: AnalysisContext): Analysis = {
    ...
    val fromCtx = aliasContext(query.from, ctx.schemas)
    val ctxWithFrom = ctx.schemas ++ fromCtx
    ...
    val (_, checkedJoin) = query.joins.foldLeft((Map.empty[Qualifier, DatasetContext[Type]], Seq.empty[typed.Join[ColumnName, Type]])) { (acc, j) =>
      ...
      val saCtx = if (j.lateral) ctxWithFrom ++ jCtx else ctx.schemas
      val subAnalysisOpt = subAnalysis(j)(ctx.copy(schemas = saCtx))
      val accCtx = subAnalysisOpt match {
        case Right(SubAnalysis(analyses, alias)) =>
          jCtx + schemasFromAnalysis(alias, analyses.outputSchema.leaf)
    ...
    val checkedDistinctOn = query.distinct match {
      case DistinctOn(exprs) => typed.DistinctOn(exprs.map(typecheck))
      case Indistinct => typed.Indistinct[ColumnName, Type]()
      case FullyDistinct => typed.FullyDistinct[ColumnName, Type]()
    }
    ...
    finishAnalysis(isGrouped, checkedDistinctOn,
                   outputs,
                   query.from,
                   checkedJoin, checkedWhere, checkedGroupBy, checkedHaving, checkedOrderBy,
                   query.limit, query.offset, query.search, checkedHint)
  }

  def finishAnalysis(isGrouped: Boolean,
                     distinct: TDistinctiveness,
...
case class SoQLAnalysis[ColumnId, Type](isGrouped: Boolean,
                                        distinct: typed.Distinctiveness[ColumnId, Type],
  ...
  def mapColumnIds[NewColumnId](f: (ColumnId, Qualifier) => NewColumnId): SoQLAnalysis[NewColumnId, Type] = {
    ...
    val mappedDistinct = distinct match {
      case typed.DistinctOn(exprs: Seq[typed.CoreExpr[ColumnId, Type]]) =>
        typed.DistinctOn(exprs.map(_.mapColumnIds(f)))
      case _: typed.Indistinct[_, _] => typed.Indistinct[NewColumnId, Type]()
      case _: typed.FullyDistinct[_, _] => typed.FullyDistinct[NewColumnId, Type]()
    }
  ...
  def mapColumnIds[NewColumnId](qColumnIdNewColumnIdMap: Map[(ColumnId, Qualifier), NewColumnId],
    ...
    val mappedDistinct = distinct match {
      case typed.DistinctOn(expr) =>
        typed.DistinctOn(expr.map(_.mapColumnIds(Function.untupled(qColumnIdNewColumnIdWithJoinsMap))))
      case _: typed.Indistinct[_, _] => typed.Indistinct[NewColumnId, Type]()
      case _: typed.FullyDistinct[_, _] => typed.FullyDistinct[NewColumnId, Type]()
    }
  ...
  private def toString(from: Option[TableName]): String = {
    ...
    val distinctStr = distinct match {
      case _: typed.Indistinct[_, _] => None
      case _: typed.FullyDistinct[_, _] => Some("DISTINCT")
      case typed.DistinctOn(exprs) =>
        itrToString(Some("DISTINCT ON ("), exprs, ", ", Some(")"))
    }
...
private class Merger[T](andFunction: MonomorphicFunction[T]) {
  ...
  private def tryMerge(a: Analysis, b: Analysis): Option[Analysis] = {
    for {
      mergedDistinct <- mergeDistinct(a, b)
      mergedAnalysis <- mergeAnalysis(a, b, mergedDistinct)
  ...
  private def mergeAnalysis(a: Analysis, b: Analysis, mergedDistinct: typed.Distinctiveness[ColumnName, T]): Option[Analysis] = {
    (a, b) match {
      case (_, b) if b.hints.exists(_.isInstanceOf[typed.NoChainMerge]) => None
      case (a, _) if (hasWindowFunction(a)) => None
      case (SoQLAnalysis(aIsGroup, _, aSelect, aFrom, Nil, aWhere, aGroup, aHaving, aOrder, aLim, aOff, None, Nil),
            SoQLAnalysis(false, _, bSelect, None, bJoins, None, Nil, None, Nil, bLim, bOff, None, Nil))
        ...
        if !(aIsGroup && bJoins.nonEmpty) && // TODO: relaxed requirement on aFrom = bFrom? is this ok?
        ...
        Some(SoQLAnalysis(isGrouped = aIsGroup,
             ...
             search = None,
             hints = Seq.empty))
      case (SoQLAnalysis(false, _, aSelect, aFrom, Nil, aWhere, Nil, None, aOrder, None, None, aSearch, Nil),
            SoQLAnalysis(false, _, bSelect, None, bJoins, bWhere, Nil, None, bOrder, bLim, bOff, None, Nil)) =>
        ...
        Some(SoQLAnalysis(isGrouped = false,
             ...
             search = aSearch,
             hints = Seq.empty))
      case (SoQLAnalysis(false, _, aSelect, aFrom, aJoins, aWhere, Nil, None, _, None, None, None, Nil),
            SoQLAnalysis(true, _, bSelect, None, bJoins, bWhere, bGroup, bHaving, bOrder, bLim, bOff, None, Nil))
        if !joinAliasCollision(a, b) =>
        ...
        Some(SoQLAnalysis(isGrouped = true,
             ...
             search = None,
             hints = Seq.empty))
      case (SoQLAnalysis(true, _, aSelect, aFrom, Nil, aWhere, aGroup, aHaving, aOrder, None, None, None, Nil),
            SoQLAnalysis(false, _, bSelect, None, Nil, bWhere, Nil, None, bOrder, bLim, bOff, None, Nil))
