...
class SoQLAnalyzer[Type](typeInfo: TypeInfo[Type], functionInfo: FunctionInfo[Type]) {
                         ...
                         search: Option[String])(implicit ctx: DatasetContext[Type]): Analysis =
  {
    ...
    val typecheck = (subscriptConverter.apply _) andThen (e => typechecker(e, Map.empty))

    val t0 = System.nanoTime()
    val checkedWhere = where.map(typecheck(_))
    val t1 = System.nanoTime()
    val checkedGroupBy = groupBy.map(_.map(typecheck(_)))
    val t2 = System.nanoTime()
    val checkedHaving = having.map(typecheck(_))
  ...
  def analyzeWithSelection(query: Select)(implicit ctx: DatasetContext[Type]): Analysis = {
    ...
    // Rewrite subscript before typecheck
    val typecheck = (subscriptConverter.apply _) andThen (e => typechecker(e, typedAliases))

    val outputs = OrderedMap(aliasAnalysis.expressions.keys.map { k => k -> typedAliases(k) }.toSeq : _*)
    val t2 = System.nanoTime()
    val checkedWhere = query.where.map(typecheck(_))
    val t3 = System.nanoTime()
    val checkedGroupBy = query.groupBy.map(_.map(typecheck(_)))
    val t4 = System.nanoTime()
    val checkedHaving = query.having.map(typecheck(_))
    val t5 = System.nanoTime()
