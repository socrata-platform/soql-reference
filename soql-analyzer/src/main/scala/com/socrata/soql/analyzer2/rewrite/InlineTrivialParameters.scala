...
class InlineTrivialParameters[MT <: MetaTypes] private (isLiteralTrue: Expr[MT] => Boolean) extends RewritePassMixin[MT] {
  ...
  def rewriteStatement(stmt: Statement, exprReplaces: ExprReplaces): Statement = {
    rewriteExpressionSubqueries(stmt, rewriteStatement(_, exprReplaces)) match {
