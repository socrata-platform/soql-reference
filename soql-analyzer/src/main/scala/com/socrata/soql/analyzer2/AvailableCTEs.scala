...
case class AvailableCTEs[MT <: MetaTypes, T](
  ctes: Map[AutoCTELabel, AvailableCTE[MT, T]]
) {
  def add[U >: T](
    label: AutoCTELabel,
    stmt: Statement[MT],
  ...
  def collect(
    defns: OrderedMap[AutoCTELabel, CTE.Definition[MT]]
  )(
    f: (AvailableCTEs[MT, T], Statement[MT]) => (T, Statement[MT])
  ): (AvailableCTEs[MT, T], OrderedMap[AutoCTELabel, CTE.Definition[MT]]) = {
    defns.iterator.foldLeft((this, OrderedMap.empty[AutoCTELabel, CTE.Definition[MT]])) { case ((aCTE, newDefns), (label, defn)) =>
  ...
  ): (B, AvailableCTEs[MT, T], OrderedMap[AutoCTELabel, CTE.Definition[MT]]) = {
    defns.iterator.foldLeft((init, this, OrderedMap.empty[AutoCTELabel, CTE.Definition[MT]])) { case ((state, aCTE, newDefns), (label, defn)) =>
...
object AvailableCTEs {
  ...
  private class Rebaser[MT <: MetaTypes, +T](ctes: Map[AutoCTELabel, AvailableCTE[MT, T]]) {
    ...
    private def rebaseAll(ctes: Map[AutoCTELabel, Statement[MT]], f: From[MT]): From[MT] =
      f.map[MT](
    ...
    private def rebaseAllAtomic(ctes: Map[AutoCTELabel, Statement[MT]], f: AtomicFrom[MT]): AtomicFrom[MT] = {
      f match {
    ...
    private def rebaseAll(ctes: Map[AutoCTELabel, Statement[MT]], s: Statement[MT]): Statement[MT] = {
      s match {
        ...
        case CTE(defns, useQuery) =>
          val (newCTEs, newDefns) = defns.iterator.foldLeft((ctes, OrderedMap.empty[AutoCTELabel, CTE.Definition[MT]])) { case ((ctes, newDefns), (label, defn)) =>
