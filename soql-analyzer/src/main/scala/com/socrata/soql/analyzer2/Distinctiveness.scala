...
sealed trait Distinctiveness[MT <: MetaTypes] extends LabelUniverse[MT] {
  ...
  private[analyzer2] def debugDoc(implicit ev: StatementDocProvider[MT]): Option[Doc[Annotation[MT]]]
}
object Distinctiveness {
  case class Indistinct[MT <: MetaTypes]() extends Distinctiveness[MT] {
    ...
    private[analyzer2] def debugDoc(implicit ev: StatementDocProvider[MT]) = None
  }
  ...
  case class FullyDistinct[MT <: MetaTypes]() extends Distinctiveness[MT] {
    ...
    private[analyzer2] def debugDoc(implicit ev: StatementDocProvider[MT]) = Some(d"DISTINCT")
  }
