// This is derived from RustCrypto's blowfish implementation:
...
private class Blowfish {
  private val s: Array[Array[Int]] = Array(
    Array(
  ...
  private val p: Array[Int] = Array(
    0x243f6a88, 0x85a308d3, 0x13198a2e, 0x03707344, 0xa4093822, 0x299f31d0, 0x082efa98, 0xec4e6c89,
    0x452821e6, 0x38d01377, 0xbe5466cf, 0x34e90c6c, 0xc0ac29b7, 0xc97c50dd, 0x3f84d5b5, 0xb5470917,
    0x9216d5d9, 0x8979fb1b
  )

  private def expand_key(key: Array[Byte]): Unit = {
    var key_pos = 0
    for(i <- 0 until 18) {
      val (xor, new_key_pos) = next_u32_wrap(key, key_pos)
      p(i) ^= xor
      key_pos = new_key_pos
    }
    var lr = (0, 0)
    for(i <- 0 until 9) {
      lr = encrypt_lowlevel(lr);
      p(2 * i) = lr._1;
      p(2 * i + 1) = lr._2;
    }
    for(i <- 0 until 4) {
  ...
  private def next_u32_wrap(buf: Array[Byte], offset0: Int): (Int, Int) = {
    var offset = offset0
    var v = 0
    for(_ <- 0 until 4) {
      ...
      v = (v << 8) | (buf(offset) & 0xff);
      offset += 1;
    }
    (v, offset)
  }

  def encrypt(n0: Long): Long = {
    val n = JLong.reverseBytes(n0)
    val (hi, lo) = encrypt_lowlevel(((n >>> 32).toInt, n.toInt));
    JLong.reverseBytes((hi.toLong & 0x0ffffffffL) << 32 | (lo.toLong & 0x0ffffffffL))
  }

  def decrypt(n0: Long): Long = {
    val n = JLong.reverseBytes(n0)
    val (hi, lo) = decrypt_lowlevel(((n >>> 32).toInt, n.toInt));
    JLong.reverseBytes((hi.toLong & 0x0ffffffffL) << 32 | (lo.toLong & 0x0ffffffffL))
  }

  private def round_function(x: Int): Int = {
    val a = s(0)(x >>> 24);
    val b = s(1)((x >>> 16) & 0xff);
    val c = s(2)((x >>> 8) & 0xff);
    val d = s(3)(x & 0xff);
  ...
  private def encrypt_lowlevel(lr: (Int, Int)): (Int, Int) = {
    var (l, r) = lr
    for(i <- 0 until 8) {
      l ^= p(2 * i);
      r ^= round_function(l);
  ...
  private def decrypt_lowlevel(lr: (Int, Int)): (Int, Int) = {
    var (l, r) = lr
    for(i0 <- 1 until 9) {
      val i = 9 - i0
      l ^= p(2 * i + 1);
...
private object Blowfish {
  def apply(key: Array[Byte]): Blowfish = {
    val blowfish = new Blowfish
