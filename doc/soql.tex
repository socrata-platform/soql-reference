...
<is-between> ::= <is-between> `IS' [`NOT'] `NULL'
 \alt <is-between> [`NOT'] `BETWEEN' <is-between> `AND' <is-between>
 \alt <order>
...
either using an aggregation function in the selection list or by the
presence of a \code{GROUP BY} clause.  In this context, expressions
refer to collections of values.  Only literals, expressions which are
...
are valid in the grouped contex.  Within the argument list of
aggregation functions, the ungrouped context is used.

If a grouped context exists in the query, all aliases are introduced
into the grouped namespace\footnote{An implementation should give a
  useful error when an out-of-scope but nonetheless existing alias is
  referred to in an ungrouped context.}.  Examples, given a table with
columns \code{x} and \code{y}:
\begin{itemize}
\item \lstinline|select x as a where a > 3| is legal.
\item \lstinline|select x as a where a > 3 group by x| is illegal---the
  alias is not in scope.
\item \lstinline|select x as a where x > 3 group by x| is legal.
\item \lstinline|select x where x > 3 group by x| is legal because the
  semi-explicit alias is not in scope, but the column itself is.
\item \lstinline|select x+7 as y, count(y) group by y| is legal but
  has what may be a surprising meaning---the parameter of the
  \code{count} function refers to the column~\code{y}, not the value
  \code{x+7}.  On the other hand, the \code{y} in the \code{GROUP BY}
  clause \emph{does} refer to the alias.  It is legal because the
  \code{x+7} is structurally equal to its alias~\code{y}.
\end{itemize}
Another way of saying this is that the \code{WHERE} clause always
exists in the ungrouped context, but the other clauses depend on
whether or not the grouped context has been introduced\footnote{An
  implementation should warn if a column which happens to share the
  name of an explicit alias is referred to in an ungrouped context
  when a grouped context is present on the query.}.  The \code{GROUP
  BY} clause itself is special; it may contain references to aliases
that themselves do not contain (even indirectly) aggregate functions.
...
\SoQL{} normal form is a notional alternate syntax for \SoQL{} which defines
an unambiguous textual representation of a \SoQL{} expression.  Normal
Form \SoQL{} consists only of literals, column references, and function
calls.  A \SoQL{} query is transformed in the following manner:
\begin{enumerate}
\item All comments are replaced by whitespace.
\item All aliases are expanded into their source expressions.
\item All keywords are normalized to upper-case.
...
\item Syntactic operators except for \lit{.} are translated into
  function calls by placing the operator before a parameter list as if
  it were a normal function.  \lit{.} is desugared into an application
  of \code{[]}.
...
\item \code{\synt{expression} IS NOT NULL} is translated into
  \code{NOT(IS_NULL(\synt{expression}))}.
...
\item \code{\synt{e1} NOT BETWEEN \synt{e2} AND \synt{e3}} is translated
  into \code{NOT(BETWEEN(\synt{e1},\synt{e2},\synt{e3}))}.
...
Examples:
\begin{itemize}
\item \code{1+1}\transformsTo\code{+(1,1)}
\item \code{`where` between 1 and 10}\transformsTo\code{BETWEEN(where,1,10)}
\item \code{(a or b) and c}\transformsTo\code{AND(OR(a,b),c)}
\item \code{a or b and c}\transformsTo\code{OR(a,AND(b,c))}
\item \code{count(*)}\transformsTo\code{count(*)}
\item \code{a::foo}\transformsTo\code{::(a,foo)}
\item \code{a[foo]}\transformsTo\code{[](a,foo)}
\item \code{a.Foo}\transformsTo\code{[](a,'Foo')}
\end{itemize}
