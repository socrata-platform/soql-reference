package com.socrata.soql.codecs
...
trait JsonColumnWriteRep extends JsonColumnCommonRep {
  ...
  protected def stdBadValue: Nothing =
    sys.error("Incorrect value passed to toJValue")
}

trait JsonColumnRep extends JsonColumnReadRep with JsonColumnWriteRep

class CodecBasedJsonColumnRep[TrueCV: JsonEncode: JsonDecode](
    val representedType: SoQLType,
    unwrapper: SoQLValue => TrueCV,
    wrapper: TrueCV => SoQLValue
) extends JsonColumnRep {
  def fromJValue(input: JValue) =
    if (JNull == input) Some(SoQLNull)
    else JsonDecode[TrueCV].decode(input).toOption.map(wrapper)
...
object JsonColumnRep {
  ...
  object TextRep
      extends CodecBasedJsonColumnRep[String](
        SoQLText,
        _.asInstanceOf[SoQLText].value,
        SoQLText(_)
      )
  object NumberRep
      extends CodecBasedJsonColumnRep[java.math.BigDecimal](
        SoQLNumber,
  ...
  object BooleanRep
      extends CodecBasedJsonColumnRep[Boolean](
        SoQLBoolean,
        _.asInstanceOf[SoQLBoolean].value,
        SoQLBoolean(_)
      )
  object ObjectRep
      extends CodecBasedJsonColumnRep[JObject](
        SoQLObject,
        _.asInstanceOf[SoQLObject].value,
        SoQLObject(_)
      )
  object ArrayRep
      extends CodecBasedJsonColumnRep[JArray](
        SoQLArray,
        _.asInstanceOf[SoQLArray].value,
        SoQLArray(_)
      )
  object BlobRep
      extends CodecBasedJsonColumnRep[String](
        SoQLBlob,
        _.asInstanceOf[SoQLBlob].value,
        SoQLBlob(_)
      )
  object PhotoRep
      extends CodecBasedJsonColumnRep[String](
        SoQLPhoto,
        _.asInstanceOf[SoQLPhoto].value,
        SoQLPhoto(_)
      )

  // Note: top-level `null's will be treated as SoQL nulls, not JSON nulls.  I think this is OK?
  object JValueRep
      extends CodecBasedJsonColumnRep[JValue](
        SoQLJson,
        _.asInstanceOf[SoQLJson].value,
        SoQLJson(_)
      )
  ...
  object JsonRep extends JsonColumnRep {
    ...
    def toJValue(input: SoQLValue): JValue = {
      input match {
        case SoQLJson(value) => JObject(Map("json" -> value))
        case SoQLNull        => JNull
        case _               => stdBadValue
      }
    }
  ...
  object FixedTimestampRep extends JsonColumnRep {
    ...
    def toJValue(value: SoQLValue): JValue =
      if (SoQLNull == value) JNull
        ...
        JString(
          SoQLFixedTimestamp.StringRep(
            value.asInstanceOf[SoQLFixedTimestamp].value
          )
        )
  ...
  object FloatingTimestampRep extends JsonColumnRep {
    ...
    def toJValue(value: SoQLValue): JValue =
      if (SoQLNull == value) JNull
        ...
        JString(
          SoQLFloatingTimestamp.StringRep(
            value.asInstanceOf[SoQLFloatingTimestamp].value
          )
        )
  ...
  object ClientNumberRep extends JsonColumnRep {
    def fromJValue(input: JValue): Option[SoQLValue] = input match {
      case JString(s) =>
        try { Some(SoQLNumber(new java.math.BigDecimal(s))) }
    ...
    def decimalToString(d: java.math.BigDecimal): String = {
      if (d.scale.abs > ScientificNotationScaleThreshold)
        d.toString() // might use scientific notation
  ...
  object ClientMoneyRep extends JsonColumnRep {
    def fromJValue(input: JValue): Option[SoQLValue] = input match {
      case JString(s) =>
        try { Some(SoQLMoney(new java.math.BigDecimal(s))) }
        catch { case _: NumberFormatException => None }
    ...
    def toJValue(value: SoQLValue): JValue =
      if (SoQLNull == value) JNull
        ...
        JString(
          ClientNumberRep.decimalToString(value.asInstanceOf[SoQLMoney].value)
        )
        ...
        case jo @ JObject(_) =>
          val map: Map[String, JValue] = jo.map {
            case (key, JString(s)) if (key == "latitude" || key == "longitude") =>
              (
                key,
                try { JNumber(new java.math.BigDecimal(s)) }
                catch { case _: NumberFormatException => JNull }
              )
        ...
        case loc: SoQLLocation =>
          JsonEncode.toJValue(loc) match {
            case jo @ JObject(_) =>
              ...
              JObject(map)
            case _ => LocationRep.toJValue(loc)
          }
  ...
  object DoubleRep extends JsonColumnRep {
    def fromJValue(input: JValue): Option[SoQLValue] = input match {
      ...
      case JString(s) =>
        try { Some(SoQLDouble(s.toDouble)) }
        catch { case _: NumberFormatException => None }
      case JNull => Some(SoQLNull)
      case _     => None
    }
  ...
  class ClientGeometryLikeRep[T <: Geometry](
      repType: SoQLType,
    ...
    def fromWkt(str: String) =
      repType.asInstanceOf[SoQLGeometryLike[T]].WktRep.unapply(str)
    def fromJson(str: String) =
      repType.asInstanceOf[SoQLGeometryLike[T]].JsonRep.unapply(str)
    def toJson(v: SoQLValue) =
      v.typ.asInstanceOf[SoQLGeometryLike[T]].JsonRep(geometry(v))
    ...
    def fromJValue(input: JValue) = {
      input match {
        ...
        case _ => {
          val geometry =
            try { geoCodec.decode(input).toOption }
            catch { case _: IOException => None }
          Try(geometry.map { geom => value(geom.asInstanceOf[T]) })
            .getOrElse(None)
        }
      }
    }
  ...
  class GeometryLikeRep[T <: Geometry](
    ...
    def fromWkt(str: String) =
      repType.asInstanceOf[SoQLGeometryLike[T]].WktRep.unapply(str)
    def fromWkb64(str: String) =
      repType.asInstanceOf[SoQLGeometryLike[T]].Wkb64Rep.unapply(str)
    def toWkb64(v: SoQLValue) =
      v.typ.asInstanceOf[SoQLGeometryLike[T]].Wkb64Rep(geometry(v))

    def fromJValue(input: JValue) = input match {
      // Fall back to WKT in case we deal with an old PG-soql-server still outputting WKT
      case JString(s) =>
        fromWkb64(s).orElse(fromWkt(s)).map(geometry => value(geometry))
      case JNull => Some(SoQLNull)
      case _     => None
    }
