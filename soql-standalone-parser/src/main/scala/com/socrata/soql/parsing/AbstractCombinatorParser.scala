...
abstract class AbstractCombinatorParser(parameters: AbstractParser.Parameters = AbstractParser.defaultParameters) extends AbstractParser with Parsers with PackratParsers {
  import parameters._

  type Elem = Token

  override def binaryTreeSelect(soql: String) = parseFull(compoundSelect, soql)
   ...
   *               *************
   */
  override def selection(soql: String): Selection = parseFull(selectList, soql)
  override def joins(soql: String): Seq[Join] = parseFull(joinList, soql)
  override def expression(soql: String): Expression = parseFull(expr, soql)
  override def orderings(soql: String): List[OrderBy] = parseFull(orderingList, soql)
  override def groupBys(soql: String): List[Expression] = parseFull(groupByList, soql)
  ...
  override def selectStatement(soql: String): NonEmptySeq[Select] = parseFull(pipedSelect, soql)
  override def unchainedSelectStatement(soql: String): Select = parseFull(select, soql) // a select statement without pipes or subselects
  override def parseJoinSelect(soql: String): JoinSelect = parseFull(joinSelect, soql)
  override def limit(soql: String): BigInt = parseFull(integer, soql)
  override def offset(soql: String): BigInt = parseFull(integer, soql)
  override def search(soql: String): String = parseFull(stringLiteral, soql)
  ...
  val select: Parser[Select] = {
    SELECT() ~> distinct ~ selectList ~ opt((FROM() ~> tableIdentifier) ~ opt(AS() ~> tableAlias)) ~ ifCanJoinList(joinList) ~ opt(whereClause) ~
      ...
      case d ~ s ~ optFrom ~ j ~ w ~ gb ~ h ~ ((ord, sr)) ~ ((lim, off)) =>
        ...
        Select(d, s, optTableName, j, w, gb.getOrElse(Nil), h, ord, lim, off, sr, None /* hint not supported */)
    }
  }
