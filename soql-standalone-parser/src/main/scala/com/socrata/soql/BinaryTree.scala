...
sealed trait Compound[T] extends BinaryTree[T] {
  ...
  override def toString: String = {
   // s"(${left.toString} $opString ${right.toString})"
    val ls = left match {
      case Compound(lop, _, _) if lop != op =>
        s"(${left.toString})"
      case _ =>
        left.toString
    }
    val rs = if (right.isInstanceOf[Compound[T]]) s"(${right.toString})" else right.toString
    s"$ls $opString $rs"
    //s"($left $opString $right)"
